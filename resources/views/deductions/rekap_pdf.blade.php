...
<body>
    ...
    <h2>REKAPITULASI PEMOTONGAN GAJI</h2>
            ...
            @forelse($members as $member)
                <tr>
                    ...
                    @foreach($types as $key => $label)
                        <td class="text-right">Rp {{ number_format($member['savings'][$key] ?? 0, 2, ',', '.') }}</td>
                    @endforeach
                    <td class="text-right">Rp {{ number_format($member['loan_principal'] ?? 0, 2, ',', '.') }}</td>
                    <td class="text-right">Rp {{ number_format($member['loan_fee'] ?? 0, 2, ',', '.') }}</td>
                    <td class="text-right">Rp {{ number_format($member['total'] ?? 0, 2, ',', '.') }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="{{ 4 + count($types) }}" class="text-center">Belum ada data pemotongan.</td>
                </tr>
            @endforelse
            <tr class="total-row">
                ...
                @endforeach
                <td class="text-right">Rp {{ number_format($summaryTotals['loan_principal'] ?? 0, 2, ',', '.') }}</td>
                <td class="text-right">Rp {{ number_format($summaryTotals['loan_fee'] ?? 0, 2, ',', '.') }}</td>
